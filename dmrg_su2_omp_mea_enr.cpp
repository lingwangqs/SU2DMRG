...
//------------------------------------------------------------------------------
void dmrg_su2::mea_enr(){
//------------------------------------------------------------------------------
  int i,j,k,m,src,des,tag=0;
  tensor_su2 vec,htmp,tconj;
  double numerator=0,denorminator,scalar,scalar1,scalar2,fac;

  //hh[ns/2-1].print();
  numerator+=hh[ns/2-1].take_trace();
  cout<<"numerator="<<numerator<<endl;
  
  hh[ns/2].multiply_singular_value(0,ww[exci]);
  hh[ns/2].multiply_singular_value(1,ww[exci]);
  numerator+=hh[ns/2].take_trace();
  cout<<"numerator="<<numerator<<endl;

  for(i=0;i<ns/2;i++)
    for(j=ns/2;j<ns;j++){
      if(hmap[i][j]!=0){
	scalar2=opr[ns/2-1][i].ss_inner_prod(opr[ns/2][j]);
	fac=coup[hmap[i][j]];
	numerator+=fac*scalar2;
      }
    }
  for(m=0;m<ns*2;m++){
    j=m%ns;
    if(plqflg[ns/2-1][j]&&plqflg[ns/2][j]){
      if(ns/2-1>=plqpos[j][0]&&ns/2<=plqpos[j][1])
	scalar2=opr[ns/2-1][plqpos[j][0]].ss_inner_prod(plqopr[ns/2][m]);
      else if(ns/2-1>=plqpos[j][1]&&ns/2<=plqpos[j][2]){
	if(m>=ns)
	  scalar2=plqopr[ns/2-1][m].ss_inner_prod(plqopr[ns/2][m]);
	else if(m<ns)
	  scalar2=plqopr[ns/2-1][m].ss_inner_prod(plqopr[ns/2][m]);
      }
      else if(ns/2-1>=plqpos[j][2]&&ns/2<=plqpos[j][3])
	scalar2=plqopr[ns/2-1][m].ss_inner_prod(opr[ns/2][plqpos[j][3]]);
      numerator-=qdelta*scalar2;	
    }
  }
  denorminator=0;
  for(i=0;i<max_dcut;i++)
    denorminator+=ww[exci][i]*ww[exci][i];
  gs_enr[exci]=numerator/denorminator;
  //gs_enr[exci]/=(double)(ly*(lx-8));
  cout<<numerator<<"\t"<<denorminator<<"\t"<<gs_enr[exci]<<endl;
  save_enr2();
  return;
}
...
//------------------------------------------------------------------------------
  ...
  if(i==0){
    ...
    hh[i].clean();
    if(!(totspin==0&&exci==0&&ky==0||totspin==2&&exci==0&&ky==(ly/2)||totspin==4&&exci==0&&ky==0))
      for(j=0;j<2;j++)
...
//------------------------------------------------------------------------------
  ...
  for(l=0;l<4+exci*2+ns*2;l++){
    ...
    if(l==0){
      ...
      if(totspin==0&&exci==0&&ky==0||totspin==2&&exci==0&&ky==(ly/2)||totspin==4&&exci==0&&ky==0)continue;
      strcat(base,"/tran-");
      sprintf(pos2,"%d",l-2);
    }
